...
const renderContacts = () => {
  ...
  contacts.forEach(contact => {
    ...
    li.innerHTML = `
      ...
      <p class ='texto1'>${contact.name}</p>
      <p class ='texto2'>${contact.phone}</p>
...
list.addEventListener('click', e => {
  ...
  if (editBtn) {
    ...
    let editvalidacion = true
    let editnumbervalidacion = true
    if (li.classList.contains('editando')) {
    
    editvalidacion = REGEX_NAME.test(name.innerHTML);
    console.log(editvalidacion);

    editnumbervalidacion = NUMBER_REGEX.test(phone.innerHTML)
    console.log(editnumbervalidacion);
    if (!editnumbervalidacion || !editvalidacion) return{
